...
def lanes():
    ...
    roi_vertices = np.array([[0, 720], [0, 600], [600, 240], [
                            720, 240], [1280, 600], [1280, 720]])
    ...
    lines = cv2.HoughLinesP(roi_image, 1, np.pi/180, 110,
    ...
    per_lines = cv2.perspectiveTransform(
        np.array(filter_lines.reshape(1, -1, 2), dtype=float), M)
    ...
    for line in per_lines:
        ...
        if abs(x1-x2) < 0.275*max(x1, x2):
            cv2.line(per_im, (int((x1+x2)/4 + max(x1, x2)/2), int((y1+y2) /
                     4 + max(y1, y2)/2)), (max(x1, x2), max(y1, y2)), (0, 255, 0), 2)
            if y1 < y2:
                filter_per_lines.append([x1, y1, x2, y2])
            else:
                filter_per_lines.append([x2, y2, x1, y1])
            cnt += 1
